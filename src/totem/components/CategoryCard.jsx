...
export const CategoryCard = ({ id, categoria, urls }) => {
  ...
  return (
    <Link to={`/shirts/${categoria.toLowerCase()}`}>
      < article
        key={id}
      ...
      >
        ...
        />
        <div
          className={`absolute inset-0 bg-gradient-to-t from-gray-900 via-gray-900/40 ease-in-out transition-all duration-300 ${mouseIn ? "opacity-100" : "opacity-0"
            }`}
        ></div>
        <h3
          className={`z-10 mt-3 text-3xl font-bold text-white transition-all duration-300 ${mouseIn ? "opacity-100" : "opacity-0"
            }`}
        >
          {categoria}
        </h3>
        <div
          className={`z-10 gap-y-1 overflow-hidden text-sm leading-6 text-gray-300 ${mouseIn ? "block delay-150" : "invisible delay-150"
            }`}
        >
